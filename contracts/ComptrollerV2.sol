...
contract ComptrollerV2 is ComptrollerV1Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function _setMintPaused(WLToken wlToken, bool state) public onlyListedMarket(wlToken) validPauseState(state) returns (bool) {
        mintGuardianPaused[address(wlToken)] = state;
        emit ActionPaused(wlToken, "Mint", state);
        return state;
    }

    function _setBorrowPaused(WLToken wlToken, bool state) public onlyListedMarket(wlToken) validPauseState(state) returns (bool) {
        borrowGuardianPaused[address(wlToken)] = state;
        emit ActionPaused(wlToken, "Borrow", state);
        return state;
    }
    ...
    function getWelAddress() public view returns (address) {
        // WEL
        return 0x854B4c305554c5fa72353e31b8480c0e5128A152;
    }
}
